...
#from source.infrastructure.simulation_service import SimulationService
...
class RobotService:
    ...
    def Configure(self, vis, debug):
        ...
        self.container.wire(modules=["__main__", "simulation_service", "llm_manager", "yolo_service", "llm_service", "tts_service"])


        self.simulationService = self.container.simulation_service()
        self.simulationService.Configure()

        self.llmManager = self.container.llm_manager()

        self.yoloService = self.container.yolo_service()
        self.yoloService.Configure("yolov9.pt", self.class_names)


    def PerformTask(self, detected_objects):
        user_input = self.stt.GetText()
        self.response = self.llmManager.handle_voice_input(user_input, detected_objects)
