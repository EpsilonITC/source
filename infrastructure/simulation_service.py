...
class SimulationService:
    ...
    @inject
    def __init__(self, llm_manager: 'LLMManager' = Provide['llm_manager'], yolo_service: 'YOLOService' = Provide['yolo_service']):
    ...
    def Configure(self, vis=True, debug=False):
        ...
        self.yolo_service.Configure("yolov9.pt", self.class_names)
    ...
    def grasp_test(self, n, vis, output, detected_objects, boxes):
        for i in range(n):
            ...
            object_names = ['003_cracker_box','005_chips', '009_gelatin_box', '011_banana', '020_hammer']
            ...
            while len(self.env.obj_ids) != 0:
                ...
                # Recognize objects using YOLO
                #prompt = input("How can I help you?")

                self.llm_manager.Configure()

                response = self.llm_manager.handle_voice_input(object_names)
                print(response)
                    ...
                    if item['command'] == "GRASP_OBJECT":
                        for i, grasp in enumerate(grasps):
                            ...
                            if closest_box is not None:
                                class_id = int(closest_box[5])
                                confidence = closest_box[4]

                                class_name = self.class_names[class_id]
                                print(f"The class name is {item['parameters']}")
